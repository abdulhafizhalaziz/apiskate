...
if (empty($stockBrg)) {
    echo "<script>alert('Data stock barang tidak ditemukan!');window.close();</script>";
    exit();
}
...
foreach ($stockBrg as $stock) {
    $pdf->Cell(10,10,$no++,0,0,'C');
    $pdf->Cell(30,10,$stock['id_barang'],0,0);
    $pdf->Cell(90,10,$stock['nama_barang'],0,0);
    $pdf->Cell(30,10,$stock['stock'],0,0,'C');
    $pdf->Cell(30,10,$stock['satuan'],0,1);
}
$pdf->Cell(190,10,'','T',1);
$pdf->Output();
?>